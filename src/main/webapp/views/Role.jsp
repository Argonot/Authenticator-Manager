		...
		<div id="admin-page-container" class="col-md-6">
			<c:choose>
				<c:when test="${ action == 'update' }">
					<c:url var="postUrl"  value="/admin/roles/${role.id}/${action}" />
			...
			<form:form action="${postUrl}" method="post" commandName="roleVO">
			  <div class="form-group">
			    <label for="ruid">RUID</label>
			    <form:input type="text" class="form-control" id="ruid" placeholder="Role Unique ID" path="id" value="${role.id}" readonly="${readonly}"/>
			    <form:errors path="id" cssClass="alert alert-danger"/>
			  </div>
			  <div class="form-group">
			    <label for="name">Nom</label>
			    <form:input type="text" class="form-control" id="name" placeholder="Nom" path="name" value="${role.name}"/>
			    <form:errors path="name" cssClass="alert alert-danger"/>
			  </div>
			  <a class="btn btn-default" href="<c:url value="/admin/roless"/>" role="button">Retour</a>
